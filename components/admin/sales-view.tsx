...
export function SalesView({
  ...
  // The sale opened in the detail drawer (row click), by charge id.
  const [openId, setOpenId] = useState<string | null>(null);
  const openSale = useMemo(() => rows.find((r) => r.id === openId) ?? null, [rows, openId]);
  ...
  return (
    <div>
            ...
            {rows.map((r) => {
              ...
              return (
                <button
                  ...
                  aria-label={`${r.customerName} · ${thb(r.amount)}`}
      ...
      <SaleDetailDrawer sale={openSale} onClose={() => setOpenId(null)} />
    </div>
  );
}
...
/**
 * Detail view for one sale: the full record, the uploaded payment slip (fetched
 * owner-gated on open via getSlip), and an Owner correction for WHEN the sale was
 * recorded (updateSaleTime). Money fields are read-only by design.
 */
function SaleDetailDrawer({ sale, onClose }: { sale: SalesRow | null; onClose: () => void }) {
  const { t, tt, lang } = useAdminLang();
  const router = useRouter();
          ...
          <dl className="mb-5 flex flex-col gap-2.5">
            <DetailRow label={t("pos_method")}>
              <span className="flex items-center gap-1.5">
                {sale.method === "promptpay" ? <PromptPayMark size={16} /> : <CashMark size={16} />}
